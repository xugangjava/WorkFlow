...
public class BpmnXMLConverter implements BpmnXMLConstants {
	...
	protected static final Logger LOGGER = LoggerFactory
			.getLogger(BpmnXMLConverter.class);
	...
	public void validateModel(InputStreamProvider inputStreamProvider)
		...
		validator.validate(new StreamSource(inputStreamProvider
				.getInputStream()));
	}
	...
	protected Schema createSchema() throws SAXException {
		...
		if (schema == null) {
			schema = factory.newSchema(BpmnXMLConverter.class.getClassLoader()
	...
	public BpmnModel convertToBpmnModel(
			InputStreamProvider inputStreamProvider, boolean validateSchema,
			boolean enableSafeBpmnXml) {
		return convertToBpmnModel(inputStreamProvider, validateSchema,
				enableSafeBpmnXml, DEFAULT_ENCODING);
	}
	...
	public BpmnModel convertToBpmnModel(
		...
		try {
			in = new InputStreamReader(inputStreamProvider.getInputStream(),
					encoding);
			...
			try {
				if (validateSchema) {
					...
					in = new InputStreamReader(
							inputStreamProvider.getInputStream(), encoding);
					xtr = xif.createXMLStreamReader(in);
				}

			} catch (Exception e) {
				throw new RuntimeException(
						"Could not validate XML with BPMN 2.0 XSD", e);
			}

			// XML conversion
			return convertToBpmnModel(xtr);
		} catch (UnsupportedEncodingException e) {
			throw new RuntimeException("The bpmn 2.0 xml is not UTF8 encoded",
					e);
		} catch (XMLStreamException e) {
			throw new RuntimeException("Error while reading the BPMN 2.0 XML",
					e);
		} finally {
			if (in != null) {
				try {
			...
			while (xtr.hasNext()) {
				...
				if (ELEMENT_DEFINITIONS.equals(xtr.getLocalName())) {
					...
					if (StringUtils.isNotEmpty(xtr.getAttributeValue(null,
							ATTRIBUTE_ID))) {
						model.addError(xtr
								.getAttributeValue(null, ATTRIBUTE_ID), xtr
								.getAttributeValue(null, ATTRIBUTE_ERROR_CODE));
					}
					...
					new IOSpecificationParser().parseChildElement(xtr,
							activeProcess, model);
					...
					if (StringUtils.isNotEmpty(xtr.getAttributeValue(null,
						...
						pool.setName(xtr
								.getAttributeValue(null, ATTRIBUTE_NAME));
					...
					if (activeSubProcessList.size() > 0) {
						parentElement = activeSubProcessList
								.get(activeSubProcessList.size() - 1);
					} else if (activeProcess != null) {
						parentElement = activeProcess;
					}
					new DocumentationParser().parseChildElement(xtr,
							parentElement, model);

				} else if (ELEMENT_SUBPROCESS.equals(xtr.getLocalName())) {

					new SubProcessParser().parse(xtr, activeSubProcessList,
							activeProcess);
					...
					new BpmnEdgeParser().parse(xtr, model);
						...
						&& ELEMENT_EXECUTION_LISTENER
								.equals(xtr.getLocalName())) {

					new ExecutionListenerParser().parseChildElement(xtr,
							activeProcess, model);

				} else {

					if (activeSubProcessList.size() > 0
							&& ELEMENT_EXECUTION_LISTENER.equalsIgnoreCase(xtr
									.getLocalName())) {
						new ExecutionListenerParser().parseChildElement(xtr,
							...
							&& ELEMENT_MULTIINSTANCE.equalsIgnoreCase(xtr
									.getLocalName())) {
					...
					} else if (convertersToBpmnMap.containsKey(xtr
							.getLocalName())) {
						if (activeProcess != null
								&& activeProcess.isExecutable()) {
	...
	private void processFlowElements(Collection<FlowElement> flowElementList,
			BaseElement parentScope) {
		for (FlowElement flowElement : flowElementList) {
			if (flowElement instanceof SequenceFlow) {
				SequenceFlow sequenceFlow = (SequenceFlow) flowElement;
				FlowNode sourceNode = getFlowNodeFromScope(
						sequenceFlow.getSourceRef(), parentScope);
				if (sourceNode != null) {
					sourceNode.getOutgoingFlows().add(sequenceFlow);
				}
				FlowNode targetNode = getFlowNodeFromScope(
						sequenceFlow.getTargetRef(), parentScope);
				if (targetNode != null) {
				...
				FlowElement attachedToElement = getFlowNodeFromScope(
						boundaryEvent.getAttachedToRefId(), parentScope);
				if (attachedToElement != null) {
					boundaryEvent
							.setAttachedToRef((Activity) attachedToElement);
					((Activity) attachedToElement).getBoundaryEvents().add(
							boundaryEvent);
				}
	...
	private FlowNode getFlowNodeFromScope(String elementId, BaseElement scope) {
		...
		if (StringUtils.isNotEmpty(elementId)) {
			if (scope instanceof Process) {
				flowNode = (FlowNode) ((Process) scope)
						.getFlowElement(elementId);
			} else if (scope instanceof SubProcess) {
				flowNode = (FlowNode) ((SubProcess) scope)
						.getFlowElement(elementId);
			}
		}
		return flowNode;
	...
	private void createXML(FlowElement flowElement, BpmnModel model,
		...
		if (flowElement instanceof SubProcess) {
			...
			Class<? extends BaseBpmnXMLConverter> converter = convertersToXMLMap
					.get(flowElement.getClass());

			if (converter == null) {
				throw new XMLException("No converter for "
						+ flowElement.getClass() + " found");
			}
	...
	private void createXML(Artifact artifact, BpmnModel model,
		...
		if (converter == null) {
			throw new XMLException("No converter for " + artifact.getClass()
					+ " found");
		}
